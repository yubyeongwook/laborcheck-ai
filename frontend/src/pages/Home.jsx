...
export default function Home() {
  ...
  const startChatWithSecretary = (userInitialPrompt) => {
    ...
    if (initialText.includes('취업규칙') || initialText.includes('계약서')) {
      ...
      // 월급/임금/일반 계산의 경우 업종 및 세무/비과세/국민연금 7월 결정액 맞춤 질문
      initialGreeting = `안녕하세요! 노무체크 AI **노무비서실장**입니다. 🎩\n\n"${initialText}" 정밀 산출 및 세무·노무 검토를 위해 아래 사항을 확인하겠습니다:\n\n1. **어떤 업종**이신가요? (예: 음식점/외식업, IT/사무직, 공장/제조업, 병원 등)\n2. **상시 근로자 수가 5인 이상**인 사업장인가요?\n3. **실제 지급액(실급여액)**과 **국세청/4대보험 세무 신고액**이 동일한가요, 아니면 차이가 있으신가요?\n4. **국민연금 매년 7월 공단 통지 기준소득월액**(상한액/하한액 기준)으로 계산할까요, 아니면 당월 과세급여 기준으로 계산할까요?\n5. **기본급에 비과세 수당**(식대 20만원, 자가운전보조금 20만원 등)을 포함하여 절세하도록 세팅해 드릴까요?\n6. 하루 총 휴게시간(식사시간+브레이크타임) 및 약정 급여는 얼마인가요?`;
    }
