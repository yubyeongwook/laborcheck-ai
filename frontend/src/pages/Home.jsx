...
export default function Home() {
  ...
  const startChatWithSecretary = (userInitialPrompt) => {
    ...
    if (initialText.includes('취업규칙') || initialText.includes('계약서')) {
      ...
      initialGreeting = `안녕하세요! 노무체크 AI **노무비서실장**입니다. 🎩\n\n"${initialText}" 정밀 산출을 위해 아래 디테일 조건을 확인하겠습니다:\n\n1. **어떤 임금 형태**이신가요? (시급제 / 일급제 / 주급제 / 정액 월급제 선택 및 약정 금액)\n2. **상시 근로자 수가 5인 이상**인 사업장인가요?\n3. **휴일근로수당을 부여할 연간 휴일 일수**는 며칠인가요? (예: 연 15일, 연 12일 등)\n4. **하계휴가(여름휴가)** 및 **동계휴가(겨울휴가)**는 며칠 부여하시나요? (예: 하계 3일, 동계 2일)\n5. 하계/동계 휴가를 기본 연차에서 차감하나요, 별도 유급휴가로 주나요, 아니면 정액 월급에 포함하여 휴일근로수당으로 산입할까요?\n6. 실제 지급액과 세무 신고액이 동일한지 및 식대 등 비과세 포함 여부를 알려주세요.`;
    }
