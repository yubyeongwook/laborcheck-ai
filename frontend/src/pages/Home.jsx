...
export default function Home() {
  ...
  const startChatWithSecretary = (userInitialPrompt) => {
    ...
    if (initialText.includes('취업규칙') || initialText.includes('계약서') || initialText.includes('복지')) {
      initialGreeting = `안녕하세요! 노무체크 AI **노무비서실장**입니다. 🎩\n\n"${initialText}" 작성을 도와드릴게요. 매장이나 회사에 딱 맞게 정해 드리기 위해 몇 가지만 편하게 말씀해 주세요:\n\n1. **어떤 업종**이시고, 사장님 제외하고 **같이 일하는 직원이 몇 분** 정도 되시나요?\n2. 부모님상이나 결혼 같은 경조사가 있을 때 **며칠 동안 유급 휴가**를 주시나요?\n3. 여름휴가나 겨울휴가는 며칠씩 주시나요?\n4. 식사 무상 제공이나 유니폼 지원 같은 **우리 매장만의 특별한 복지**가 있으신가요?`;
