...
`;
    ...
    const numericResultBlock = `
[계산 결과 숫자 명시 필수]
법정수당 항목별 점검 결과 안에 "가산수당을 지급해야 합니다" 같은 추상적 서술만으로 끝내지 말고, 문서에 실제로 기재된 시급/일급/월급 금액을 근거로 원 단위 숫자를 계산해서 별도 소제목("■ 계산 결과 요약")으로 정리하십시오. 예시 형식:
- 확인된 시급(또는 환산시급): OOO원 (2026년 최저임금 10,320원 대비 충족/미달)
- 주휴수당(1일 기준): OOO원 = 환산시급 × 1일 소정근로시간
- 연장근로수당(1시간당): OOO원 = 환산시급 × 1.5 (5인 이상) 또는 × 1.0 (5인 미만)
- 야간근로수당(1시간당 가산분): OOO원 = 환산시급 × 0.5 (5인 이상만)
- 휴일근로수당(1시간당, 8시간 이내분): OOO원 = 환산시급 × 1.5 (5인 이상만)
- 연차유급휴가 미사용수당(1일당): OOO원 = 환산시급 × 1일 소정근로시간 (5인 이상만)
반드시 지킬 것:
1. 위 숫자는 문서에 실제로 적힌 시급/일급/월급을 기준으로 매번 새로 계산하십시오. 사업장마다, 기재된 기본급마다 결과 숫자가 달라야 하며, 미리 정해둔 예시 숫자를 그대로 베껴 쓰지 마십시오.
2. 문서에 시급/일급/월급 등 구체적 금액이 전혀 없어 계산할 수 없다면 숫자를 지어내지 말고 "정확한 계산을 위해 시급(또는 일급·월급) 금액 정보가 필요합니다"라고 명시하십시오.
3. 계산 과정(수식)도 함께 보여주어 사용자가 직접 검산할 수 있게 하십시오.
`;
...
${rulesComparisonBlock}${specialWorkingHoursBlock}${workerCategoryBlock}${companySizeBlock}${wageTypeBlock}${allowanceChecklistBlock}${numericResultBlock}
[요구사항]
...
7. "■ 계산 결과 요약" 섹션에 문서의 실제 시급/일급/월급을 근거로 계산한 원 단위 수당 금액을 반드시 포함
