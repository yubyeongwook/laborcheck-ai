...
// 노무 상담 신청 이메일 알림 발송 API 엔드포인트 (Resend 사용)
const consultationRateLimitStore = new Map();
const CONSULTATION_RATE_LIMIT_WINDOW_MS = 60 * 60 * 1000; // 1시간
const CONSULTATION_RATE_LIMIT_MAX = 5; // 시간당 최대 5회

function checkConsultationRateLimit(key) {
  const now = Date.now();
  const timestamps = (consultationRateLimitStore.get(key) || []).filter(t => now - t < CONSULTATION_RATE_LIMIT_WINDOW_MS);
  if (timestamps.length >= CONSULTATION_RATE_LIMIT_MAX) {
    consultationRateLimitStore.set(key, timestamps);
    return false;
  }
  timestamps.push(now);
  consultationRateLimitStore.set(key, timestamps);
...
app.post('/api/send-consultation-email', async (req, res) => {
  try {
    ...
    if (!name || !email || !message) {
      return res.status(400).json({ error: '이름, 이메일, 상담 내용은 필수입니다.' });
    }

    const clientIp = req.ip || req.socket?.remoteAddress || 'unknown';
    if (!checkConsultationRateLimit(`ip:${clientIp}`)) {
      return res.status(429).json({ error: '요청이 너무 많습니다. 잠시 후 다시 시도해 주세요.' });
    }

    const resendApiKey = process.env.RESEND_API_KEY || '';
    const adminEmail = process.env.ADMIN_NOTIFY_EMAIL || 'aigoid1203@gmail.com';

    if (!resendApiKey) {
      console.warn('⚠️ RESEND_API_KEY가 설정되지 않아 상담 신청 이메일 발송을 건너뜁니다.');
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'LaborCheck AI <onboarding@resend.dev>',
        to: [adminEmail],
        reply_to: email,
        subject: `[노무체크 AI] 새 상담 신청 - ${name}`,
        html: `
          <h3>새로운 노무 상담 신청이 접수되었습니다.</h3>
    ...
    res.status(500).json({ error: '이메일 발송 도중 오류가 발생했습니다.', details: error.message });
  }
});
